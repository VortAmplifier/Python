...
class Product:						# defining class Product
	def __init__(self, pnumber, pname):		# __init__ method 
...
def main():												# define main function
	...
	if select == 1:											# if 1 is selected
		productNumber = int(input('Enter product number: '))					# ProductNumber assigned from input
		productName = input('Enter product name: ')						# ProductName assigned from input
		productLL.insert(productNumber, productName)						# call productLL method insert
		flag = input('Do you wish to continue (Y/N): ')
		while flag.lower() != 'n':								# flag is to lowercase
			if flag.lower() == 'y':								# flag is to lowercase
				productNumber = int(input('Enter product number: '))			# ProductNumber assigned from input
				productName = input('Enter product name: ')				# ProductName assigned from input
				productLL.insert(productNumber, productName)				# call productLL method insert
				flag = input('Do you wish to continue (Y/N): ')				# flag has a value Y or N
			else:
				print('Invalid choice')							# print Invalid choice
	
	productLL.printInventory()									# print inventory

if __name__ == '__main__':										# if standalone program or a module
	main()
